...
const sharedFn = (options) => {
        ...
        if (OnlyDropDown == true && controlvalidationlist) {
            ...
            if (AlldropdownList.length > 0) {
                let dropdownInitializer = jqDropdownInitializer({
                    ...
                    url: `/${lang}/DropDown/GetDropDownValues`,
                    lang: lang,
                });
        ...
        for (const item of duallistcontrollist) {
            ...
            if (contrains.controlType === 'DUAL_LIST') {
                ...
                await new Promise((resolve, reject) => {
                    ...
                    }).Post("DropDown/GetDropDownValues", data);
                });
            }
        }
    }
    ...
    const resetduallist = (dualist) => {
        $.each(dualist, function (i, dropdown) {
            if (dropdown.constraint.controlJsonConfig) {
                ...
                jqClientAdvanced(options).Post("DropDown/GetDropDownValues", data);
    ...
    const editMode = () => {
        ...
        if (typeof btnAddContentId !== 'undefined') {
            $(`#${btnAddContentId}`).attr("disabled", "disabled");
        }
    ...
    const viewMode = () => {
        ...
        $(`#${btnAddContentId}`).removeAttr("disabled");
    ...
    const actionCellClick = (event, cell) => {
        ...
        if (clazzList.contains('view')) {
            if (tableId.startsWith("fieldTable-")) {
                popupname = "Field";
            }
            if (tableId=="ActionStatusConfigtable") {
                popupname = "ActionStatusConfiguration";
                
            }
            if (tableId == "ActionConditiontable") {
                popupname = "ActionCondition";

            }
            if (tableId == "ReminderReminderConditiontabulator") {
                popupname = "ReminderCondition";
            }
            if (tableId == "AllocationCalculationtabulator") {
                popupname = "AllocationCalculation";
            }
    ...
    const getActionTemplate = (pkId, dynamicaction) => {
        ...
        if (typeof IsDelete != 'undefined') {
            deleteTmpl = IsDelete == "True" ?
                `<span class="delete pointer" title="${getUiControlText('ADMIN_TOOLTIP_DELETE')}"><i class="delete fa fa-trash-can"></i></span>`
                : '';
        }
        var editTmpl = '';
        if (typeof IsEdit != 'undefined') {
            editTmpl = IsEdit == "True" ?
                `<span class="edit pointer" title="${getUiControlText('ADMIN_TOOLTIP_EDIT')}"><i class="edit fa fa-edit"></i></span>`
                : '';
        }
        var ViewTmpl = '';
        if (typeof IsView != 'undefined') {
            ViewTmpl = IsView == "True" ?
                `<span class="view pointer" title ="${getUiControlText('ADMIN_TOOLTIP_VIEW')}"><i class="view fa fa-eye"></i></span>`
                : '';
        }
        return `
...
const ajaxSetup = (config) => {
    ...
    const handleAjaxError = (jqXHR, textStatus, errorThrown) => {
        ...
        switch (jqXHR.status) {
            case 400:
                sharedFn().DisplayAlert(jqXHR.responseJSON.Message, 'error');
                break;
            case 401:
                sharedFn().DisplayAlert(jqXHR.responseJSON.Message, 'error');
                break;
            case 403:
                ...
                break;
            case 404:
                sharedFn().DisplayAlert('Not Found', 'error');
                break;
            default:
        }
    }
    ...
    let defaults = {
        ...
        beforeSend: function (xhr, options) {
            sharedFn().CoverSpin(true);
            //options.url = decodeURIComponent(baseApiUrl()) + options.url;
        },
    ...
    result.HandleAjaxError = handleAjaxError;
