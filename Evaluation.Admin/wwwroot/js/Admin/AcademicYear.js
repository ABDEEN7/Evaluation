...
const loadData = (isScroll) => {
    ...
    jqClientAdvanced(options).Get("AcademicYear/GetAllAcademicYear".concat('?Page=', currentPage));
...
const deleteData = (id, event, cell) => {
    ...
    notificationUtil.confirmation({ title: sharedFn().GetUiControlText('ADMIN_WARNING_DELETE'), okText: sharedFn().GetUiControlText('DELETE_BUTTON'), cancelText: sharedFn().GetUiControlText('ADMIN_CANCEL') }, result => {
        ...
        jqClientAdvanced(options).Post("AcademicYear/DeleteAcademicYear".concat('?Id=', id));
...
$(document).ready(function () {
    ...
    function SaveAcademicYear() {
        ...
        if (id) {
            url = "AcademicYear/UpdateAcademicYear";
        } else {
    ...
    $("#btn-submit").click(function (e) {
        ...
        if (sharedFn().NewvalidateForm("form-control", sharedFn().GetUiControlText('ADMIN_CNTRL_REQUIRED'), sharedFn().GetUiControlText('ADMIN_MSG_MAX_CHAR_LENGTH'), sharedFn().GetUiControlText('ADMIN_MSG_MIN_CHAR_LENGTH'))) {
            ...
            if ($("#AcademicYearIsCurrent").prop("checked")) {
                var DepartmentId = $("#AcademicYearDepartmentId").val();
                const Defaultoptions = {
                    success: function (response) {
                        if (response) {
                            if (response.id != $("#Id").val()) {
                                notificationUtil.confirmation({ title: sharedFn().GetUiControlText('ADMIN_WARNING_CURRENT'), okText: sharedFn().GetUiControlText('CONFIRM_BUTTON'), cancelText: sharedFn().GetUiControlText('ADMIN_CANCEL') }, result => {
                                    var tablerow = table.getRows()
                                        .filter(row => row.getData().isCurrent == true && row.getData().departmentId == DepartmentId)[0];
                                    if (tablerow != undefined)
                                    table.updateRow(tablerow, { isCurrent: false });
                                    SaveAcademicYear();
                ...
                jqClientAdvanced(Defaultoptions).Get("AcademicYear/GetCurrentAcademicYear".concat('?DepartmentId=', DepartmentId));
            }
            else {
                SaveAcademicYear();
            }
