...
const loadData = (reqData, isScroll) => {
    ...
    jqClient.Post({
        url: "ControlValidation/GetAllControlValidation",
...
function ClearControlByPage() {
    var controltype = $("#ControlValidationControlType").val();
    if (controltype == "DROPZONE" || controltype == "FILEUPLOAD") {
        $("#ControlValidationFileExtention").parent().show();
        $("#ControlValidationFileCount").parent().show();
        $("#ControlValidationFileSize").parent().show();
    }
    else {
        $("#ControlValidationFileExtention").parent().hide();
        $("#ControlValidationFileCount").parent().hide();
        $("#ControlValidationFileSize").parent().hide();
    }
}

$("#ControlValidationControlType").on("change", event => {
    var data = event.target.value;
    if (data) {
        if (data == "DROPZONE" || data == "FILEUPLOAD") {
            $("#ControlValidationFileExtention").parent().show();
            $("#ControlValidationFileCount").parent().show();
            $("#ControlValidationFileSize").parent().show();
        }
...
const searchColsDef = () => {
    return [
        {
            field: 'title',
            header: sharedFn().GetUiControlText('ControlValidationTitle'),
            type: 'TEXT_BOX'
        }
    ];
};
...
$(document).ready(function () {
    ...
    $("#btn-submit").click(function (e) {
        ...
        if (sharedFn().NewvalidateForm("form-control", sharedFn().GetUiControlText('ADMIN_CNTRL_REQUIRED'), sharedFn().GetUiControlText('ADMIN_MSG_MAX_CHAR_LENGTH'), sharedFn().GetUiControlText('ADMIN_MSG_MIN_CHAR_LENGTH'))) {
            ...
            jqClientAdvanced(options).PostFormData("ControlValidation/UpdateControlValidation", requestdata);
