...
const searchColsDef = () => {
    return [
        {
            field: 'orgClassId',
            header: sharedFn().GetUiControlText('SchoolsOrgClass'),
            type: 'DROPDOWN',
            collections: []
        },
        {
            field: 'title',
            header: sharedFn().GetUiControlText('SchoolsTitle'),
            type: 'TEXT_BOX'
        }
    ];
};
...
const deleteData = (id) => {
    ...
    notificationUtil.confirmation({ title: sharedFn().GetUiControlText('ADMIN_WARNING_DELETE'), okText: sharedFn().GetUiControlText('DELETE_BUTTON'), cancelText: sharedFn().GetUiControlText('ADMIN_CANCEL') }, result => {
        ...
        jqClientAdvanced(options).Post("Schools/DeleteSchools".concat('?Id=', id));
...
const gettreedata = () => {
                ...
                if (data) {
                    ...
                    if (ddlElm) {
                        ...
                        $(dropdown).select2({
                            ...
                            allowClear: true,
                            data: ddlData,
                            placeholder: sharedFn().GetUiControlText('SchoolsOrgClass'),
    ...
    jqClientAdvanced(options).Get("Schools/GetOrgClass");
...
$(document).ready(function () {
    ...
    $("#btn-submit").click(function (e) {
        ...
        if (sharedFn().NewvalidateForm("form-control", sharedFn().GetUiControlText('ADMIN_CNTRL_REQUIRED'), sharedFn().GetUiControlText('ADMIN_MSG_MAX_CHAR_LENGTH'), sharedFn().GetUiControlText('ADMIN_MSG_MIN_CHAR_LENGTH'))) {
            ...
            if (id) {
                url = "Schools/UpdateSchools";
            } else {
                url = "Schools/SaveSchools";
