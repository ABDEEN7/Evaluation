...
const searchColsDef = () => {
    return [
        ...
        {
            field: 'title',
            header: sharedFn().GetUiControlText('PartyTypeTitle'),
            type: 'TEXT_BOX'
        }
    ];
};
...
$("#PartyTypeIsEmployeePartyType").on("change", function (e) {
    SetIsEmployeePartytype(this.checked);
});
$("#PartyTypeCanViewEntityScholarships").on("change", function (e) {
    if (this.checked) {
        $("#PartyTypeSector").parent().show();
        $("#PartyTypeEntityContract").parent().show();
    }
    else {
        $("#PartyTypeSector").parent().hide();
        $("#PartyTypeEntityContract").parent().hide();
    }
});
const deleteData = (id) => {
    if (!id) return;
    ...
    notificationUtil.confirmation({ title: sharedFn().GetUiControlText('ADMIN_WARNING_DELETE'), okText: sharedFn().GetUiControlText('DELETE_BUTTON'), cancelText: sharedFn().GetUiControlText('ADMIN_CANCEL') }, result => {
        ...
        jqClientAdvanced(options).Post("PartyType/DeletePartyType".concat('?Id=', id));
...
function SetIsEmployeePartytype(checked) {
    if (checked) {
        $("#PartyTypeCanViewAllRequests").removeAttr("disabled");
        $("#PartyTypeCanViewAllRequests").prop("checked", false);
        $("#PartyTypeCanViewAllEvaluations").removeAttr("disabled");
        $("#PartyTypeCanViewAllEvaluations").prop("checked", false);
    }
    else {
        $("#PartyTypeCanViewAllRequests").attr("disabled", "disabled");
        $("#PartyTypeCanViewAllRequests").prop("checked", false);
        $("#PartyTypeCanViewAllEvaluations").attr("disabled", "disabled");
        $("#PartyTypeCanViewAllEvaluations").prop("checked", false);
    }
}
