...
const getLookup = () => {
    const options = {
        success: function (result) {
            if (result) {
                ...
                if (data) {
                    const { PageName } = data;
                    const ddlData = PageName.map(item => (
                        {
                            id: item.pageName,
                            text: item.pageName
                        }
                    ));
                    const ddlElm = document.querySelector(`[data-key="pageName"]`);
                    if (ddlElm) {
                        ...
                        $(dropdown).select2({
                            ...
                            allowClear: true,
                            data: ddlData,
                            placeholder: sharedFn().GetUiControlText('UiControlPageName'),
    ...
    jqClientAdvanced(options).Get("UiControl/GetPageNameList");
...
const searchColsDef = () => {
    return [
        {
            field: 'pageName',
            header: sharedFn().GetUiControlText('UiControlPageName'),
            type: 'DROPDOWN',
            collections: []
        },
        {
            field: 'title',
            header: sharedFn().GetUiControlText('UiControlTitle'),
            type: 'TEXT_BOX'
        }
    ];
};
...
$(document).ready(function () {
    ...
    $("#btn-submit").click(function (e) {
        ...
        if (sharedFn().NewvalidateForm("form-control", sharedFn().GetUiControlText('ADMIN_CNTRL_REQUIRED'), sharedFn().GetUiControlText('ADMIN_MSG_MAX_CHAR_LENGTH'), sharedFn().GetUiControlText('ADMIN_MSG_MIN_CHAR_LENGTH'))) {
            ...
            jqClientAdvanced(options).PostFormData("UiControl/UpdateUiControl", requestdata);
