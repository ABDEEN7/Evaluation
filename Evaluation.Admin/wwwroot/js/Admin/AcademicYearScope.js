...
const loadData = (isScroll) => {
    ...
    jqClientAdvanced(options).Get("AcademicYearScope/GetAllAcademicYearScope".concat('?Page=', currentPage));

};
function ClearControlByPage() {
    var optionvalue = $("#AcademicYearScopeScopeId").val();
    if (optionvalue) {
        $("#AcademicYearScopeParentId option[value='" + optionvalue + "']").prop("disabled", true);
    }
}
$("#AcademicYearScopeScopeId").on("change", function () {
    $("#AcademicYearScopeParentId option").prop("disabled", false);
    var optionvalue = $("#AcademicYearScopeScopeId").val();
    if (optionvalue == null || optionvalue =="") {
        var datavalue = $("#AcademicYearScopeScopeId").attr("data-value");
        if (datavalue != "" || optionvalue != null)
    ...
    if (optionvalue) {
        ...
        $("#AcademicYearScopeParentId").val('').trigger("change");
    }
...
const deleteData = (id, event, cell) => {
    ...
    notificationUtil.confirmation({ title: sharedFn().GetUiControlText('ADMIN_WARNING_DELETE'), okText: sharedFn().GetUiControlText('DELETE_BUTTON'), cancelText: sharedFn().GetUiControlText('ADMIN_CANCEL') }, result => {
        ...
        jqClientAdvanced(options).Post("AcademicYearScope/DeleteAcademicYearScope".concat('?Id=', id));
...
$(document).ready(function () {
    ...
    $("#btn-submit").click(function (e) {
        ...
        if (sharedFn().NewvalidateForm("form-control", sharedFn().GetUiControlText('ADMIN_CNTRL_REQUIRED'), sharedFn().GetUiControlText('ADMIN_MSG_MAX_CHAR_LENGTH'), sharedFn().GetUiControlText('ADMIN_MSG_MIN_CHAR_LENGTH'))) {
            ...
            if (id) {
                url = "AcademicYearScope/UpdateAcademicYearScope";
            } else {
                url = "AcademicYearScope/SaveAcademicYearScope";
