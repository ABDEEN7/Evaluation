...
namespace Evaluation.Services.Mappers.Admin
{
    public class SchoolLevelProfile : Profile
    {
        public SchoolLevelProfile()
        {
            CreateMap<SchoolLevel, SchoolLevelDTO>()
                ...
                .ForMember(dest => dest.School, opt => opt.MapFrom<SchoolResolver, Guid?>(src => src.SchoolId))
                .ForMember(dest => dest.EducationLevel, opt => opt.MapFrom<EducationLevelResolver, Guid?>(src => src.EducationLevelId));

        }

    }

    public class SchoolResolver : IMemberValueResolver<object, object, Guid?, string?>
    {
        private readonly UnitOfWork _uow;
        private readonly RequestInfo _requestInfo;

        public SchoolResolver(UnitOfWork uow, RequestInfo requestInfo)
        {
            _uow = uow;
        ...
        public string? Resolve(object source, object destination, Guid? sourceMember, string? destMember, ResolutionContext context)
        {
            ...
            var status = _uow.GetRepository<School>()
                         .GetAllNonDeleted()
    ...
    public class EducationLevelResolver : IMemberValueResolver<object, object, Guid?, string?>
    {
        private readonly UnitOfWork _uow;
        private readonly RequestInfo _requestInfo;

        public EducationLevelResolver(UnitOfWork uow, RequestInfo requestInfo)
        {
            _uow = uow;
            _requestInfo = requestInfo;
        }
        ...
        {
            if (!sourceMember.HasValue) return "";
            var status = _uow.GetRepository<EducationLevel>()
                         .GetAllNonDeleted()
                         .FirstOrDefault(x => x.Id == sourceMember);
