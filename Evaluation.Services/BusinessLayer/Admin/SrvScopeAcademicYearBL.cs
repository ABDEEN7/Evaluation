...
public class SrvScopeAcademicYearBL : AdminBase
{
    ...
    public async Task<ScopeAcademicYearDTO> SaveScopeAcademicYear(ScopeAcademicYearDTO message)
    {
        ...
        if (await CheckScopeAcademicYearExisitsByDepartmentId(message.ScopeId, message.DepartmentId))
        {
            throw new BusinessException(ExceptionMessage.DuplicatedScopeWithSameDepartment);
        }
        if(message.ScopeId == message.ScopeAcademicYearScopeParentId)
        {
            throw new BusinessException(ExceptionMessage.ScopeAndParentSame);
        }
    ...
    private Task<bool> CheckScopeAcademicYearExisitsByDepartmentId(Guid scopeId, Guid? departmentId)
    {
        return uow.GetRepository<ScopeAcademicYear>().GetAllNonDeleted().AnyAsync(x => x.DepartmentId == departmentId && x.ScopeId == scopeId);
    }
}
