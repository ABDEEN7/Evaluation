...
namespace Evaluation.Services.BusinessLayer.API.FormBuilderLayer.Srvices
{
...

    {
        ...
        public async Task<Guid?> GetUserIdByRequestIdAsync(Guid reqId)
        {
        ...
        public async Task<WebAppRequestsDTO> GetRequestsAsync(Guid userId, FilterRequestsDTO model)
        {
            ...
            if (!isMinistry)
            {
                ...
                filteredResult = await FilteredRequestsAsync(uow, isMinistry, requestsQuery, model);
            }
            else
            {
        ...
        public async Task<ServiceRequestDTO> GetRequestDetailsAsync(Guid id)
        {
            ...
            if (isMinistry)
            {
                if(request.Scholarship==null)
                var (requestCountryId, requestUniversityId) = await GetRequestCountryAndUniversityAsync(request.Id);
                else


                if (user.Id != request.CreateById && !await ValidateMinistryUserAccessAsync(userId.Value, Module?.Id, request.Scholarship.CountryId, request.Scholarship.UniversityId, request.Id))
                {
            ...
            var actionsTask = SrvActionStatusConfiguration.GetActionsByStatus(request.ServiceId, request.StatusId, request.Id, request.ScholarshipId, lang);
            var ApplicationStatusGroupTask = SrvStatus.GetApplicationStatusGroup(request.StatusId, request.ServiceId, lang);

            await Task.WhenAll(attachmentsTask, actionTransactionsTask, applicantTask, actionsTask, ApplicationStatusGroupTask, RequestFieldsValueTask);
            ...
            var dto = new ServiceRequestDTO
            {
               ...
               StatusGroup = ApplicationStatusGroupTask.Result,
                ScholarshipNo = request.Scholarship?.ScholarshipNumber,
        ...
        public async Task<bool> HasAccessToRequestAsync(Guid requestId, Guid userId)
        {
            ...
            if (isMinistry)
            {
                ...
                if (await ValidateMinistryUserAccessAsync(userId, module?.Id, requestCountryId, requestUniversityId, request.Id))
                    return true;
        ...
        {
            string Lang = _requestInfo.Lang;
            var AllCountryTask = GetAllCountriesAsync();
            var AlluniversityTask = GetAllUniversitiesAsync();

            var AllCountry = await AllCountryTask;
            var Alluniversity = await AlluniversityTask;
            await requests.ParallelForEachAsync(async (item) =>
            {
                if (item.CountryName == null || item.CountryName =="")
                {
                    var (requestCountryId, universityId) = await GetRequestCountryAndUniversityAsync(item.Id!.Value);
                    var country = requestCountryId != null ? AllCountry.FirstOrDefault(x => x.Id == requestCountryId) : null;
                    var university = universityId != null ? Alluniversity.FirstOrDefault(x => x.Id == universityId) : null;

                    item.CountryCode = country?.Code;
                    item.CountryName = country != null ? (Lang == "ar" ? country.NameAr : country.NameEn) : null;
                    item.UniversityCode = university?.Code;
                    item.UniversityName = university != null ? (Lang == "ar" ? university.NameAr : university.NameEn) : null;
                }
        ...
        {
            // 1. Try to get the full list from cache
            var data = cacheDataProvider.GetFromCache<List<University>>(ConstantKeys.WebAppCacheTableName.CACHE_UNIVERSITY);

            // 2. If cache miss, fetch from DB and cache the result
            if (data == null)
            {
                using var scopedUow = serviceScopeFactory.CreateScopedUow();
                var repository = scopedUow.GetRepository<University>();

                data = await repository.GetAllQueryFiltered().ToListAsync();

                await cacheDataProvider.SetToCache(ConstantKeys.WebAppCacheTableName.CACHE_UNIVERSITY, data);
            }
            return data.ToList();
        }
        ...
        private  async Task<IQueryable<ServiceRequestDTO>> GetRequestsForMinistryUserAsync(UnitOfWork uow, Guid userId,SystemModule module, string lang, string timeFormat,string dateFormat)
        {
            ...
            return baseQuery.Select(c => new ServiceRequestDTO
            {
                ...
                SchStatus = c.Scholarship != null && c.Scholarship.SchStatus != null
                    ...
                    : "",
                CountryName = c.Scholarship.Country != null
                    ? (lang == "ar" ? c.Country.NameAr : c.Country.NameEn)
                    : "",
                ...
                CountryId = c.CountryId ,
                UniversityId = c.UniversityId ,
                UniversityName =  c.University != null
                    ? (lang == "ar" ? c.University.NameAr : c.University.NameEn)
                    : ""
            });
        }
        private IQueryable<ServiceRequest> ApplyCountryUniversityFilter(IQueryable<ServiceRequest> baseQuery,IEnumerable<UserPartyTypeDTO> userPartyTypeData)
        {
            if (userPartyTypeData.Any(x => x.PartyTypeCountyUniversity.Any()))
            {
                //if (isInitialService)
                //{
                //    return  ApplyCountryUniversityFilterForInitial(baseQuery, userPartyTypeData);
                //}
                //else
                //{
                    return  ApplyCountryUniversityFilterForNonInitial(baseQuery, userPartyTypeData);
                //}
            }

            return baseQuery;
        }
        private IQueryable<ServiceRequest> ApplyCountryUniversityFilterForNonInitial(IQueryable<ServiceRequest> baseQuery, IEnumerable<UserPartyTypeDTO> userPartyTypeData)
        {
            var userAccess = userPartyTypeData
                .SelectMany(x => x.PartyTypeCountyUniversity)
                .ToList();

            // Unrestricted countries (no university restriction)
            var unrestrictedCountryIds = userAccess
                .Where(x => x.UniversityId == null)
                .Select(x => x.CountryId.ToString())
                .Distinct()
                .ToList();

            // Restricted country-university pairs
            var restrictedPairs = userAccess
                .Where(x => x.UniversityId != null)
                .Select(x => x.CountryId.ToString() + "-" + x.UniversityId.ToString())
                .Distinct()
                .ToList();

            var query = baseQuery;

            if (unrestrictedCountryIds.Any())
            {
                var unrestrictedQuery = query.Where(sr =>
                    sr != null &&
                    sr!.CountryId! != Guid.Empty &&
                    unrestrictedCountryIds.Contains(sr.CountryId!.ToString()));

                if (restrictedPairs.Any())
                {
                    var restrictedQuery = query.Where(sr =>
                        sr.CountryId != Guid.Empty &&
                        sr.UniversityId != Guid.Empty &&
                        restrictedPairs.Contains(
                            sr.CountryId.ToString() + "-" +
                            sr.UniversityId.ToString()
                        )
                    );

                    query = unrestrictedQuery.Union(restrictedQuery);
                }
                else
                {
        ...
        {
            // Country/University based filtering
            if (isAllowedToViewAllRequests && userPartyTypeData.Any(x => x.PartyTypeCountyUniversity.Any()))
            {
                return  ApplyCountryUniversityFilter(baseQuery, userPartyTypeData);
            }

            // Entity contract based filtering
            if (userPartyTypeData.Any(x => x.PartyTypeEntityContract.Any()))
            {
                return ApplyEntityContractFilter(baseQuery, userPartyTypeData);
            }
        ...
        private IQueryable<ServiceRequest> ApplyEntityContractFilter(IQueryable<ServiceRequest> baseQuery, IEnumerable<UserPartyTypeDTO> userPartyTypeData)
        {
            var allowedEntityIds = userPartyTypeData
                .SelectMany(x => x.PartyTypeEntityContract)
                .Select(x => x.EntityContractId)
                .Distinct()
                .ToList();

            return baseQuery.Where(c =>
                c.Scholarship != null &&
                c.Scholarship.EntityContractId != null &&
                allowedEntityIds.Contains(c.Scholarship.EntityContractId.Value));
        }
        private IQueryable<ServiceRequest> ApplyUserSpecificFilter(IQueryable<ServiceRequest> baseQuery, Guid userId)
        {
            return baseQuery
                ...
                //||  TO DO : Move To First Query
                //    (c.Service != null && c.Service.RequestShowPartyType != null &&
                //        c.Service.RequestShowPartyType.Any(x => userInfo.PartyTypes.Contains(x.PartyTypeId))));
        }
        
        private async Task<bool> ValidateMinistryUserAccessAsync(Guid userId, Guid? moduleId, Guid? requestCountryId, Guid? requestUniversityId, Guid? requestId)
        {
            if (moduleId is null)
            {
                return false;
            ...
            if (userPartyData.Any(pt => pt.CanViewAllRequests && pt.PartyTypeCountyUniversity.Count == 0))
            {
                return true;
            }

            var hasCountryUniversityAccess = userPartyData.Any(pt => pt.PartyTypeCountyUniversity.Any(ctu =>
                                                ctu.CountryId == requestCountryId &&
                                                (ctu.UniversityId == null || ctu.UniversityId == requestUniversityId)
                                            ));
            if (hasCountryUniversityAccess)
                return true;
            else
            {
        ...
        public async Task<List<FormGroupDTO>> GetRequestFieldsValueAsync(ServiceRequest request)
        {
            ...
            var userGender = await srvUser.GetUserGenderByuserId(request.StudentId);
        ...
        public async Task<List<ServiceRequestFieldsValue>> GetRequestFieldsValueAsync(Guid? requestId)
        {
            var RequestFieldsValue = await serviceScopeFactory.CreateScopedUow()
                                        ...
                                        .GetAllActiveNonDeleted()
                                        .Include(x => x.Field)
                                        .Include(x => x.Field!.MappingSystemField)
        ...
        public async Task<(Guid? requestCountryId, Guid? universityId)> GetRequestCountryAndUniversityAsync(Guid requestId)
        {
            using var uow = serviceScopeFactory.CreateScopedUow();

            var request = await uow.GetRepository<ServiceRequest>()
                .GetByIDActiveNonDeleted(requestId);

            if (request == null)
                return (null, null);

            return (request.CountryId, request.UniversityId);
        }
            ...
            if (model != null)
            {
                ...
                if (model.PageNumber != null)
                {
                    ...
                    if (model.OderByAction == true)
                    {
                        ...
                        await requestList.ParallelForEachAsync(async (item) =>
                        {
        ...
        public async Task<IList<FieldTransactionDTO>> GetFieldHistoryAsync(Guid fieldId, Guid requestId, string lang = "ar")
        {
            ...
            var transactions = await serviceScopeFactory.CreateScopedUow()
                ...
                .Select(c => new FieldTransactionDTO()
                {
                    ...
                    User = lang == "ar" ? c.CreateBy!.FullNameAr : c.CreateBy!.FullNameEn
                })
                .ToListAsync();
        ...
        private async Task<string> ResolveDropDownTextAsync( string lang,string rawValue, Guid dropDownTypeId, Guid? userGender, Guid? scholarshipId) 
        {
            if (Guid.TryParse(rawValue, out var singleId) && singleId != Guid.Empty)
            {
                var v = await SrvDropdown.GetDropDownValue(lang, singleId, dropDownTypeId, userGender, scholarshipId);
            ...
            {
                var v = await SrvDropdown.GetDropDownValue(lang, id, dropDownTypeId, userGender, scholarshipId);
