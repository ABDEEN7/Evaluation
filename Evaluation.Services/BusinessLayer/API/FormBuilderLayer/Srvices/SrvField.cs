...
using Scholarship.DAL.ConstantKeys;
...
{
    public class SrvField(SrvAttachments SrvAttachments, INSISServices INSISServices, IServiceScopeFactory serviceScopeFactory, CacheDataProvider cacheDataProvider, UnitOfWork uow, LoggingServices loggingServices, IMapper mapper, UserInfo userInfo, IServiceProvider serviceProvider, RequestInfo _requestInfo)
            : ApiBase(serviceScopeFactory, cacheDataProvider, uow, loggingServices, mapper, userInfo, serviceProvider, _requestInfo)
    {
        ...
        public async Task<SystemField?> GetSysFieldsByIdsAsync(Guid SysfieldId)
        {
            var fields = await serviceScopeFactory.CreateScopedUow()
                .GetRepository<DAL.Models.FormBuilder.SystemField>()
        ...
        public async Task<List<Field>> GetFieldsByActionId(Guid actionId,Guid ServiceId)
        {
            ...
            if (data == null)
            {
                ...
                .Include(f => f.MappingSystemField)
            ...
            return data
                .Where(f => f.ActionsStepsField.Any(x => x.ServiceActionId == actionId) && f.FormGroup!.FormGroupTypeId != ConstantKeys.FormGroupTypeKeyIds.List)
                .ToList();
        }
        public async Task<List<Field>> GetFieldsListByActionIdAsync(Guid? serviceId, Guid? SystemModuleId = null)
        {
            ...
            var FieldsList = data
                 ...
                 .Where(f => f.FormGroup.FormGroupTypeId == ConstantKeys.FormGroupTypeKeyIds.List)
                 .ToList();
            return FieldsList;
        }
        public async Task<string?> GenerateJsonSchemaForFormGroupList(Guid? formGroupListId, List<DAL.Models.FormBuilder.Field> FieldsList, string RenderType = null!,bool schfield=false)
        {
            ...
            if (FieldsList.Any())
            {
                ...
                var schema = new
                {
                    ...
                    {
                        FieldId = (schfield==true? f.MappingSystemFieldId:f.Id),
                        Value = "",
        ...
        public List<ExtractedObjectDto> ExtractFieldValues(List<Field> fieldList, string jsonData)
        {
            ...
            foreach (var entry in jsonDataList)
            {
                ...
                foreach (var item in entry)
                {
                    ...
                    extractedObject.Fields!.Add(new FieldListValueDto
                    {
                        FieldBackendName = matchingField.MappingSystemField?.BackendName!,
                        Value = value
                    });
                }
