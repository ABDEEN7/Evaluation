...
#pragma warning disable S2325
    public class FieldValidatorBL(CacheDataProvider cacheDataProvider)
        ...
        public List<string> ValidateNotEqualFields( IList<FieldValueDTO> fields, List<FieldAttributeValue> fullAttributes, StudentUser Student, string lang)
        {
            var errors = new List<string>();

            foreach (var field in fields)
            {
                var relatedAttributes = fullAttributes
                    .Where(a => a.FieldId == field.FieldId && a.AttributeKey.ToLower().StartsWith("notequal"))
                    .ToList();

                if (!relatedAttributes.Any()) continue;

                foreach (var attr in relatedAttributes)
                {
                    var attrVal = attr.AttributeValue?.Trim();
                    if (string.IsNullOrEmpty(attrVal)) continue;

                    string targetValue = string.Empty;

                    switch (attrVal)
                    {
                        case "studentAccountEmail":
                            targetValue =  Student.Email; break;
                        case "studentAccountMobile":
                            targetValue = Student.Mobile!; break;
                        case "NotScholarshipStudentQID":
                            targetValue = Student.QID; break;
                        default:
                            var targetField = fields.FirstOrDefault(f => $"[{f.FieldId}]" == attrVal);
                            if (targetField != null)
                                targetValue = (targetField.Value ?? "").Trim();
                            break;
                    }

                    var fieldValue = (field.Value ?? "").Trim();

                    if (!string.IsNullOrEmpty(fieldValue) && fieldValue == targetValue)
                    {
                        var customMessage = lang == "ar"
                            ? attr.MessageAr ?? $"لا يجب أن يكون الحقل {field.FieldName} مساويًا للقيمة المستهدفة."
                            : attr.MessageEn ?? $"Field {field.FieldName} must not match the target value.";
